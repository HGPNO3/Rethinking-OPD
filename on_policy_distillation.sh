...
set -e
# Printing the command is the safe review default; execution is explicit.
OPD_DRY_RUN=${OPD_DRY_RUN:-1}

# Configure logging when running outside SBATCH.
if [ "$OPD_DRY_RUN" != 1 ] && [ -z "$SLURM_JOB_ID" ]; then
...
export OUTLINES_CACHE_DIR=${OUTLINES_CACHE_DIR:-$HOME/.cache/outlines/opd-review}
...
if [ "$OPD_DRY_RUN" != 1 ]; then
    ray start --head
fi
run_training() {
    if [ "$OPD_DRY_RUN" = 1 ]; then
        python3 -c 'import json,sys; print(json.dumps({"status":"command_only_not_hydra_composed_or_executed", "argv":["python3",*sys.argv[1:]]},indent=2))' "$@"
    else
        python3 "$@"
    fi
}
...
run_training -m verl.trainer.main_ppo \
