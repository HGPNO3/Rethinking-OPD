...
def main():
 p=argparse.ArgumentParser();p.add_argument('--records',required=True);p.add_argument('--output',required=True);p.add_argument('--previous');p.add_argument('--limit',type=int,default=1000);p.add_argument('--snapshot-id',required=True);p.add_argument('--model',default='/home/ecs-user/models/Qwen3-4B');a=p.parse_args()
 ...
 try:
  ...
  t=time.perf_counter();base=AutoModelForCausalLM.from_pretrained(a.model,local_files_only=True,trust_remote_code=False,dtype=torch.bfloat16,device_map={'':0},attn_implementation='sdpa')
  tok=AutoTokenizer.from_pretrained(a.model,local_files_only=True);vocab=set(tok.get_vocab().values())
  model=PeftModel.from_pretrained(base,Path(a.previous)/'adapter',is_trainable=True) if a.previous else get_peft_model(base,LoraConfig(r=32,lora_alpha=64,lora_dropout=0,bias='none',target_modules='all-linear',task_type='CAUSAL_LM'));model.eval();model.config.use_cache=False;model.gradient_checkpointing_enable(gradient_checkpointing_kwargs={'use_reentrant':False});model.enable_input_require_grads()
  ...
  for r in rows:
   assert r['target_ids'] and r['target_ids'][-1] in [151643,151645]
   assert all(type(x)is int and x in vocab for x in r['target_ids'])
   assert not {151667,151668}.intersection(r['target_ids'])
