...
PROMPT_VERSION = "two_teacher_reference_verbatim_goal_positive_ig_v1_20260917"
...
def prompt_binding():
 return hashlib.sha256(json.dumps([SELECTION_VERSION,PROMPT_VERSION,COMMON_GUIDANCE,SPECIALTIES,REFERENCE_GUIDANCE,inspect.getsource(teacher_messages)],sort_keys=True).encode()).hexdigest()
...
def validate_reference_record(r):
 ...
 expected=opd_messages(r['visible_messages'],r['specialty'],reference)
...
def strict_action(text):
 ...
 if not isinstance(x,dict) or set(x)!={'action_type','argument'} or x['action_type'] not in ACTIONS or not isinstance(x['argument'],str):
...
class Client:
 ...
 async def request(self,payload,kind):
  ...
  for attempt in range(3):
   try:
    async with self.http.post(self.url,json={'model':self.model,**payload}) as r:
     body=await r.json()
 ...
 async def generate(self,messages,seed,kind):
  ...
  response=await self.request({'prompt':ids,'temperature':0.7,'top_p':1.0,'top_k':-1,'repetition_penalty':1.0,'presence_penalty':0.0,'frequency_penalty':0.0,'max_tokens':remaining,'seed':seed,'logprobs':0,'return_token_ids':True,'skip_special_tokens':False,'stop_token_ids':sorted(self.eos)},kind)
  ...
  rec={'prompt_token_ids':ids,'generated_token_ids':tokens,'behavior_logprobs':lp,'behavior_temperature':0.7,'behavior_probability_mode':'processed_logprobs','model':self.model,'seed':seed,'finish_reason':choice['finish_reason'],'raw_text':self.tok.decode(tokens,skip_special_tokens=False)}
...
def opd_messages(messages,specialty,reference_action):
 # Keep reference in teacher-only data, never in the student's public history.
 reference=strict_action(action_key(reference_action))
 return [{'role':'system','content':COMMON_GUIDANCE+' '+SPECIALTIES[specialty]+' '+REFERENCE_GUIDANCE},
         {'role':'user','content':json.dumps({'target_time_visible_messages':messages,'reference_action':reference},ensure_ascii=False)}]

class Pilot:
 def __init__(self,args,student,teacher):self.args=args;self.student=student;self.teacher=teacher;self.rows=[];self.dialogues=[];self.selected=[]
 ...
 async def inspect(self,session,messages,original,key):
  row={'selection_version':SELECTION_VERSION,'prompt_version':PROMPT_VERSION,'prompt_binding':prompt_binding(),'id':key,'original':original,'candidates':[],'selected':None,'status':'pending'}; self.rows.append(row)
  async def propose(d):
   try:
  ...
  if row['selected']:
   ...
   teacher_context=opd_messages(messages,row['selected'],reference)
   raw,teacher=await gather_cancel(self.student.score(original['prompt_token_ids'],ids,'old_student_raw_score'),self.teacher.score(self.teacher.render(teacher_context),ids,'opd_teacher_score'))
   ...
   sample.update(prompt_version=PROMPT_VERSION,prompt_binding=prompt_binding(),reference_action=reference,reference_action_sha256=hashlib.sha256(action_key(reference).encode()).hexdigest(),visible_messages=copy.deepcopy(messages),teacher_messages=teacher_context,teacher_prompt_sha256=hashlib.sha256(json.dumps(teacher['prompt_token_ids']).encode()).hexdigest())
   validate_reference_record(sample)
