...
def validate_bindings(
    ...
    if actual_method in {"two_teacher_non_thinking_online_v1", "two_teacher_reference_online_v2", "two_teacher_prefix_only_online_v3"}:
        ...
        expected = {"thinking": False, "specialties": ["expression", "turn"],
                    "target_nodes": document.get("authorized_target_nodes", 1000), "teacher_candidates_per_specialty": 1,
    ...
    if model_config.get("target_nodes") != 1000:
        if model_config.get("target_nodes") != 3000 or document.get("authorized_target_nodes") != 3000:
            errors.append("Only explicitly approved cumulative 3000-node extension is supported")
        if not model_config.get("continuation") or document.get("continuation") != model_config.get("continuation"):
            errors.append("Continuation lineage lacks identical explicit approval binding")
