...
class TestBatch(unittest.TestCase):
 def test_duplicate_key_failure_is_reproduced_and_never_executed(self):
  body='{"action_type":"speak","argument":"A","argument":"B"}'
  d={'id':'a','complete':False,'events':[], 'failure':{'reason':'duplicate JSON key','receipt':{'raw_text':body+'<|im_end|>','finish_reason':'stop'}}}
  before=copy.deepcopy(d)
  r=validate_batch({'status':'partial'},[d],['a'],[])
  self.assertEqual(before,d);self.assertEqual(r['validated_selected_nodes'],0)
  self.assertFalse(r['invalid_outputs_repaired']);self.assertFalse(r['resampled_for_quality'])
  with self.assertRaises(ValueError):validate_batch({'status':'partial'},[d],['a'],[{'id':'a:0:0'}])
  for text,finish in [('{}<|im_end|>','stop'),(body+'<|im_end|>','length'),('<think>'+body+'<|im_end|>','stop'),(body,'stop')]:
   bad=copy.deepcopy(d);bad['failure']['receipt'].update(raw_text=text,finish_reason=finish)
   with self.assertRaises(ValueError):validate_batch({'status':'partial'},[bad],['a'],[])
