...
cmd=[r['teacher_vllm'],'serve',str(folder),'--served-model-name','teacher','--host',r['teacher_bind'],'--port','18014','--tensor-parallel-size',str(r['teacher_tp']),'--dtype','bfloat16','--max-model-len',str(cfg['context']),'--max-num-seqs',str(r['teacher_max_num_seqs']),'--max-num-batched-tokens','4096','--gpu-memory-utilization',str(r['teacher_gpu_memory_utilization']),'--generation-config','vllm','--logprobs-mode','processed_logprobs','--enforce-eager','--no-async-scheduling','--seed',str(cfg['seed'])]
