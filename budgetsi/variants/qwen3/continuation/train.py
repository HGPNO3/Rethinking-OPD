...
class Controller:
 ...
 def run_mode(self,mode,batches=None):
  out=self.out/mode;out.mkdir(exist_ok=True);pool=load_json(ROOT/'inputs.json')['scenes'];limit=self.cfg['target_nodes'] if mode=='formal' else 10000
  ...
  while True:
   ...
   if summary['status']!='completed':raise RuntimeError('Incomplete dialogue batch; preserve and inspect, no silent training')
 ...
 def run(self):
  try:
   ...
   self.command([COL,'-m','unittest','test_runner','test_train','test_reference_opd','test_continuation'],self.out/'tests.log',120)
   from continuation import validate_continuation
   atomic(self.out/'continuation_verified.json',validate_continuation(self.cfg,self.out))
   # Revalidate inherited acceptance without rewriting the immutable parent result.
   acceptance_ids,_,acceptance_updates=receipts(self.out/'acceptance')
   assert len(acceptance_updates)==2 and acceptance_ids, 'Inherited acceptance must include two valid updates'
