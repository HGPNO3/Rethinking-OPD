...
def reviewed_format_failure(f):
 ...
 # Reviewed model-output boundaries end only this dialogue. They do not excuse
 # transport, CUDA, nonfinite probabilities, or unknown collector failures.
 reason=f.get('reason')
 if reason=='context_exhausted':return bool(r.get('prompt_token_ids'))
 if reason=='truncated_or_no_eos':
  return bool(r.get('generated_token_ids')) and (r.get('finish_reason')=='length' or not text.endswith(('<|im_end|>','<|endoftext|>')))
 if reason=='Generated thinking marker in nonthinking action':return '<think>' in text or '</think>' in text
 if reason=='Tokens after action EOS':
  body=text.removesuffix('<|im_end|>').removesuffix('<|endoftext|>')
  return '<|im_end|>' in body or '<|endoftext|>' in body
 ...
 if f.get('reason') in {'duplicate JSON key','nonfinite JSON'}:
  # Reproduce the collector's strict rejection; never choose a duplicate value.
  from budgetsi.social_protocol.runner import strict_action
  try:strict_action(body)
  except ValueError as e:return str(e)==f['reason']
  return False
 try:json.loads(body)
...
def validate_batch(summary, dialogues, scene_ids, records):
 ...
 return {'policy':'retain_valid_executed_prefix_nodes_log_reproduced_json_syntax_schema_or_duplicate_v4','normal_dialogues':sum(bool(d.get('complete')) for d in dialogues),'invalid_schema_dialogues':failures,'invalid_outputs_repaired':False,'resampled_for_quality':False,'validated_selected_nodes':len(records)}
